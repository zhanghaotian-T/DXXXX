...
class ResultDeal(object):
        ...
        with open(file_path) as f:
            for index, line in enumerate(f):
                ...
                if ("ACLR_EUTRA_L1" in line) and (index - self.aclr_l1_pre != 8):
                    ...
                    self.report.append(self.sn + ';' + self.ant + ';' + self.test_model + ';' + self.channel + ';' + 'ACLR_L1;' + aclr_l1)
                    self.aclr_l1_pre = index
                if ('ACLR_EUTRA_U1' in line) and (index - self.aclr_u1_pre != 8):
                    aclr_u1 = re.findall(r'[0-9]+.[0-9]+', line)[0]
                    self.report.append(self.sn + ';' + self.ant + ';' + self.test_model + ';' + self.channel + ';' + 'ACLR_U1;' + aclr_u1)
                ...
                if ('Tx Mean Power Average' in line) and (self.test_model == 'LTE_ETM1_1'):
                    ...
                    self.report.append(self.sn + ';' + self.ant + ';' + self.test_model + ';' + self.channel + ';' + 'TX_MEAN_POWER;' + tx_mean_power)
                if 'EVM Average' in line:
                    evm = re.findall(r'[0-9]+.[0-9]+', line)[0]
                    self.report.append(self.sn + ';' + self.ant + ';' + self.test_model + ';' + self.channel + ';' + 'EVM;' + evm)
                if 'CCDF Power Level at 0.1%' in line:
                    ccdf = re.findall(r'[0-9]+.[0-9]+', line)[1]
                    self.report.append(self.sn + ';' + self.ant + ';' + self.test_model + ';' + self.channel + ';' + 'CCDF 0.1%;' + ccdf)
    ...
    def save_report_log(self, result_list: list):
        ...
        final_result = pd.DataFrame(result_list_dataframe, columns=['SN', 'ANT', 'Test_Model', 'Frequency', 'Test_Case', 'Result'])
